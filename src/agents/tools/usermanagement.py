...
def createNewUser(names:List = 'user', password :str = "") -> Dict : 
    ...
    try:
        ...
        return {
            'success':True, #try this with True
            'result':response,
    ...
    try:
        ...
        for user in users:
            if os == 'windows':
                ...
                cmd = ['sudo', 'usermod', '-U', user]  # fixed: was 'usermod' '-U' (missing comma → string concat bug)
...
def deleteUser(user:str) -> Dict :
    ...
    try:
        os = checkOS()
        if os == 'windows':
            # Remove-LocalUser is PowerShell-only; use 'net user /delete' for subprocess
            cmd = ['net', 'user', user, '/delete']
        elif os == 'linux' or os == 'darwin' :
            cmd = ['sudo', 'userdel', '-r', user]  # fixed: was 'group' (wrong variable)
